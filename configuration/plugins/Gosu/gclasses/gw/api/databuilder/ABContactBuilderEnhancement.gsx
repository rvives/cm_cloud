...
enhancement ABContactBuilderEnhancement<T extends ABContact, B extends ABContactBuilder<T, B>> : gw.api.databuilder.ABContactBuilder<T, B> {
  ...
  reified function withABOfficialIds(officialID : ABOfficialID) : B {
    withABOfficialIds(ExistingBean.wrap(officialID))
    return this as B
  }

  reified function withABOfficialIds(abOfficialID : ValueGenerator< ABOfficialID >) : B {
    this.addArrayElement(ABContact.Type.TypeInfo.getProperty("ABOfficialIDs"), abOfficialID)
    return this as B
  }
