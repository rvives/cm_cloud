...
uses gw.webservice.ab.ab5040.abcontactapi.ABContactAPIAddressSearch
uses gw.webservice.ab.ab5040.abcontactapi.ABContactAPIProximitySearchParameters
uses gw.webservice.ab.ab5040.abcontactapi.IABContactAPISearchCriteria
...
@Export
@gw.xml.ws.annotation.WsiExportable( "http://guidewire.com/ab/ws/gw/webservice/ab/ab5040/abcontactapi/ABContactAPISearchCriteria" )
final class ABContactAPISearchCriteria implements IABContactAPISearchCriteria {
  ...
  public var OfficialIDs      : ABContactAPIABOfficialIDParameters[]
  ...
  override function toSearchCriteria(b : Bundle) : ABContactSearchCriteria {
    ...
    if (this.OfficialIDs != null) {
      for(officialId in this.OfficialIDs) {
        if(officialId.OfficialIDType == null) {
          var errorMessage = DisplayKey.get("Webservice.ABContactAPI.Contact.MissingOfficialIDType")
          _logger.error(errorMessage)
          throw new SOAPSenderException(errorMessage)
        }
        criteria.addToOfficialIDs(new ABContactSearchCriteriaOfficialID(b) {
          :OfficialIDType = officialId.OfficialIDType,
          :OfficialIDValue = officialId.OfficialIDValue,
          :State = officialId.State
        })
      }
    }
    return criteria
